...
### 5-3. 최근 등장 일수 (calcRecent 함수, 2026-09-17 변경)

- 데이터 마지막 날 기준 최근 28일 중 등장한 날 수를 센다(연속 여부는 보지 않음)
- 그 날들의 평균 비율 계산, 마지막 등장 후 14일이 지났으면 등급을 한 단계 내림
- 바꾼 이유: 2026-09-17부터 일간 데이터가 매일 자동으로 들어온다(video-auto 통계 수집기). 실제 일간 데이터에서는 롱테일 검색어가 하루 걸러 들어와 연속이 끊겨 최근 검색어가 목록에 거의 안 나왔다. 예전 연속 일수는 주간·월간 파일이 모든 날짜에 복사돼 부풀려진 값이었다
...
```
Priority Score = 최근 28일 등장일수 × 평균유입비율(%) × 갭배수
...
| 분류 | 조건 |
|---|---|
| 즉시 작성 | 최근 28일 중 6일+ 등장 AND 최근 14일 안에 등장 AND 커버되지 않음 |
| 작성 고려 | 최근 28일 중 3~5일 등장(또는 6일+지만 최근 14일엔 없음) AND 커버되지 않음 |
| 모니터링 | 최근 28일 중 2일 등장(또는 3~5일이지만 최근 14일엔 없음) AND 커버되지 않음 |
| 과거 기록 | 최근 28일 안에 없고 과거에 2일+ 등장 (접힌 칸) |
...
| `calcRecent(dateRatios, latestDate)` | 최근 28일 등장일수 + 평균비율 + 등급 |
...
## 2026-09-17 변경: 주제 묶기(대상 + 의도)와 글 매칭

**왜 바꿨나** — 예전 묶기는 임베딩 유사도 0.8↑ 또는 편집거리 2↓인 검색어를 사슬처럼 이어 붙였다(union-find).
A~B, B~C가 이어지면 A~C도 한 묶음이 돼서 비만치료제 검색어 969개가 카드 한 장('이미 커버됨')이 됐고,
그 안의 개별 질문이 전부 묻혔다. 롱테일 질문은 문구가 매번 달라 하루짜리로 흩어져 화면에 안 나왔다(유입의 52%).
글 매칭은 '추천·고르는법·영양제' 같은 일반어만 겹쳐도 부분 커버로 판정해 엉뚱한 글이 연결됐다.

**지금** (`groupKeywords`, `matchTopicToPosts`)
- 검색어를 **대상(성분·약 이름) + 의도**로 풀어 그 조합을 한 주제로 센다. 의도: 병용·부작용·내성/중단·복용법·처방/급여·가격/구매·고르기·공급·후기/경과·효능·성분/정체·일반
- 대상은 ① keyword-deep-dive의 뿌리·제품 사전 ② 검색어들에 2번 이상 나온 단어에서 찾는다. 붙여 쓴 검색어는 부분 문자열로, 오타는 편집거리 1로 붙인다.
  조사는 떼어 낸 꼴이 실제로 있는 단어일 때만 뗀다(마운자로→마운자 방지). 별칭은 `ENTITY_ALIAS`.
- 글 매칭은 대상 이름 기준: 제목에 대상 + 의도까지 있으면 covered, 제목에 대상만(또는 본문 키워드에만) 있으면 partial, 어디에도 없으면 gap.
- 임베딩(OpenAI)은 이제 묶기에 쓰지 않는다(설정 화면은 남아 있음).
- 하루짜리 롱테일이 주제 단위로 합산돼 숨겨지는 유입이 52% → 28%.

**데이터 입력** — 2026-09-17부터 미니PC(video-auto 통계 수집기)가 매일 09:10에 전날 '지표 다운로드 → 유입분석 · 일간' 엑셀을 받아
`inflow_history.json`에 올린다. 수동 업로드는 예비용.
